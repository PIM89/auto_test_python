...
def test_7z_check_path():
    ...
    assert res_arch and res_view_file2, "test_7z_check_path fail"

def test_7z_hash():
    res_arch = checkout_positive("cd {}; 7z a ../out/arx2".format(folder_in), "Everything is Ok")
    res_crc32 = subprocess.run("cd {}; crc32 arx2.7z".format(folder_out), shell=True, encoding="utf-8", stdout=subprocess.PIPE)
    crc32_hash = res_crc32.stdout.upper()
    res_compare_hash = checkout_positive("cd {}; 7z h arx2.7z".format(folder_out), crc32_hash)
    assert res_arch and res_compare_hash, "test_7z_hash fail"